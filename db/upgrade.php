...
function xmldb_enrol_flexaccess_upgrade($oldversion): bool {
    ...
    if ($oldversion < 2026081730) {
        ...
        $availablefrom = new xmldb_field('availablefrom', XMLDB_TYPE_INTEGER, '10', null,
            XMLDB_NOTNULL, null, '0', 'enrolperiod');
        if (!$dbman->field_exists($table, $availablefrom)) {
            $dbman->add_field($table, $availablefrom);
        }

        $availableuntil = new xmldb_field('availableuntil', XMLDB_TYPE_INTEGER, '10', null,
            XMLDB_NOTNULL, null, '0', 'availablefrom');
        if (!$dbman->field_exists($table, $availableuntil)) {
            $dbman->add_field($table, $availableuntil);
        }

        $maxparticipants = new xmldb_field('maxparticipants', XMLDB_TYPE_INTEGER, '10', null,
