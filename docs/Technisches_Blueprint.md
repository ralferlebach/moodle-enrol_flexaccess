# enrol_flexaccess — Technisches_Blueprint

**Verantwortung:** System-/Kategorie-/Kurs-Policies, automatische Einschreibung, Rollen/Gruppen, Einschreibedauer und Teilnehmerlisten-Policy.
...
## Zusatzanforderung: System-/Kurs-Zugangsschlüssel
...
`enrol_flexaccess_instance` enthält `temporaryaccesskeymode` (`inherit|course`) und optional `temporaryaccesskeyhash` (Hash, niemals Klartext). Es gibt bewusst kein entsprechendes Feld in `enrol_flexaccess_policy`: Kategorieebenen dürfen Zugangsarten einschränken, aber keine gemeinsam geteilten Zugangsschlüssel definieren. Der System-Hash liegt in `config_plugins` des Enrolment-Plugins.
